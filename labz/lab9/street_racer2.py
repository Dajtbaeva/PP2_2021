...
pg.mixer.init()
pg.mixer.music.load("./music/background.wav")
pg.mixer.music.play(-1)
bg = pg.image.load("./images/AnimatedStreet.png")
game_over = pg.image.load("./images/gameover.jpg")
...
class Enemy(pg.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pg.image.load("./images/Enemy.png") # загружаем картинку
...
class Player(pg.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pg.image.load("./images/Player.png") # загружаем картинку
        self.rect = self.image.get_rect()
...
class Coin(pg.sprite.Sprite):
    def __init__(self):
        ...
        self.image = pg.image.load(f'./images/coin{self.r}.png') # по рандомному числу занружаем картинку монетки
        self.rect = self.image.get_rect()
...
while is_running:
    ...
    if pg.sprite.spritecollideany(p, enemies):
        ...
        time.sleep(0.8) # даем время на проигрывание звука порожения
